...
public record PoissonDiskSampler(int width, int height, int maxSamples) {
    ...
    public List<Vec2i> generate(List<Room.Instance> instances, boolean[] roomCache, String key, Random random) {
        int cells = width * height;
        List<Vec2i> points = new ArrayList<>(cells);
        List<Vec2i> active = new ArrayList<>(cells);
        ...
        // Choose 5 random starting points
        for (int i = 0; i < 5; i++) {
            Vec2i startingPoint = new Vec2i(random.nextInt(width), random.nextInt(height));
            if (!active.contains(startingPoint)) {
                active.add(startingPoint);
            }
        }

        int roomSamples = 0;
        final int expectedRooms = cells / 16;
        int consecutiveFailures = 0;
        final int maxFailures = cells / 8;
        ...
        while (!active.isEmpty() && roomSamples < expectedRooms && consecutiveFailures < maxFailures) {
            ...
            Vec2i activeCenter = active.get(random.nextInt(active.size()));
            boolean hasMapping = RoomRegistry.contains(key);

            // Use a default room in case the key doesn't have a mapping
            Room.Instance newInstance = hasMapping ? RoomRegistry.forKey(key, random).newInstance(random) : Room.DEFAULT.newInstance(random);
            float spacing = hasMapping ? RoomRegistry.getSpacing(key) : 1.0f;
            byte roomWidth = newInstance.height();
            byte roomHeight = newInstance.width();
            double minSeparation = this.dynSeparation(roomWidth, roomHeight, spacing, cells, expectedRooms - roomSamples);
            ...
            for (int i = 0; i < maxSamples; i++) {
                ...
                Vec2i newCenter = activeCenter.add(separate(dir.x), separate(dir.y));
                // Continue if the new room doesn't collide with another
                if (this.isValid(newCenter, roomWidth, roomHeight, roomCache)) {
                    // Add it to the generation list if it had a mapping
                    if (hasMapping) {
                        ++roomSamples;
                        consecutiveFailures = 0;
    ...
    private boolean isValid(Vec2i candidate, byte roomWidth, byte roomHeight, boolean[] grid) {
        int startX = candidate.x() - 1;
        int endX = candidate.x() + roomWidth + 1;
        int startY = candidate.y() - 1;
        int endY = candidate.y() + roomHeight + 1;
